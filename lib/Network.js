...
export function SearchLocation(query, long, lat) {
    return new Promise((res, rej) => {
        fetch (`${MAPS_SEARCH_URL}json?query=${query}&location=${long},${lat}&key=${MAPS_API_KEY}`).then(_response => _response.json()).then(_response => {
            res(_response);
        }).catch(err => {
